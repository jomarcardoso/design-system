...
Thirty-two checks against a `patterns.json` that already exists. Run it after
...
One line per failure, plus a count. **The three counts must add up to 32.** If
the arithmetic does not close, the review has not finished.

```
patterns.json review — 29 pass, 1 fail, 2 unverifiable
