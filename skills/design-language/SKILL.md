...
Twenty-one questions, six blocks — two of them apply to one colour school only,
...
| `elevation` | borders | derived — `editorial-premium`, and `dwell` does not move it |
| `surfaceSeparation` | lines | derived from `elevationCarrier` |
...
[`references/derivations.md`](references/derivations.md) turns the twenty-one
...
those go in the read-back as a block, with their provenance:
