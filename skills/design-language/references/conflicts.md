...
### Elevated surfaces, and a loud posture

`surfaceModel: elevated` dims the page so the content can be the lit thing.
`posture: loud` puts a high-contrast fill on the action. Both are bids for the
same attention, and run together they produce a screen where the card is
shouting and the button is shouting over it — which reads as fatigue rather than
as emphasis, and reads that way fastest at long dwell.

**The softening move is to let the model carry the emphasis and quieten the
posture.** An elevated model has already solved the problem loud was for: the
thing that matters is the lit one. Propose `balanced`, and reserve the loud fill
for the single action that ends a flow.

Where the client keeps both, the cost is concrete and goes in the document: the
accent budget shrinks to one element per screen, because two loud things at
different depths is the combination nobody can scan.

### A colour-critical workspace, and a playful archetype

`colorCriticalWorkspace: true` exists to keep the interface from competing with
the image the user is judging: neutral chrome, low contrast, near-zero pigment.
`archetype: playful-expressive` brings decorative shadows, saturated chrome and
a filled icon set.

**This one is not a matter of taste and should be said so.** The flag is
physiological — a bright interface changes the viewer's adaptation state and the
image is judged wrong. A playful chrome around a grading view is not a bold
choice, it is a broken instrument.

The softening move is to keep the archetype for everything OUTSIDE the editing
view — onboarding, settings, marketing, the account area — and hold the flag over
the workspace alone. That is a real and common split, and it is worth recording
as two contexts rather than as one compromise.

### Recessed surfaces, and a dark theme generated by inversion

`surfaceModel: recessed` groups content by going DARKER than the page. Under the
dark theme there is nowhere darker to go: the page is already near the floor of
the ramp, and a panel one step down is either invisible or black.

**This is not a conflict the client created and it is not one they can answer.**
It is `derivations.md` §T, and it resolves in the build rather than in the
interview: under dark, a recessed model groups by stepping one rung UP. Raise it
only to say what is happening, because the DOCUMENT has to record it in words
that hold in both themes — *"grouped content steps away from the page toward the
middle of the ramp"* — and a document that says "panels are darker" is true of
one theme and a lie in the other.

### An elevated model, and a request for dividers everywhere

A client who has been looking at a bordered system will ask for the lines back
after seeing an elevated one. The lines are redundant there — the tone
difference between a lighter card and the page already draws the boundary — and
adding them is the graphic noise that makes a quiet product look busy for a
reason nobody can name.

**The softening move is to grant the line where the tone difference is
smallest** and nowhere else: the header rule, the row separators inside a table,
the edge of an aside that shares the page tone. Those are the places a divider is
still doing work. Around a card that is already lighter, it is not.
